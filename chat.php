...
if(!isSet($_REQUEST['action'])){
	...
	if(empty($_REQUEST['do'])){
		...
		if(isSet($_REQUEST['name'])) logout_chatter($_REQUEST['name']);
...
function send_setup(){
	...
	if(!isSet($_REQUEST['session'])) echo hidden('session', $U['session']);
	...
	echo '<option value="0"'; if(!$dismemcaptcha) echo ' selected'; echo ">$I[enabled]</option>";
	echo '<option value="1"'; if($dismemcaptcha) echo ' selected'; echo ">$I[onlyguests]</option>";
	...
	foreach($C['bool_settings'] as $setting){
		...
		echo '<option value="0"'; if(!$value) echo ' selected'; echo ">$I[disabled]</option>";
		echo '<option value="1"'; if($value) echo ' selected'; echo ">$I[enabled]</option>";
		echo '</select></td></tr></table></td></tr>';
	}
	thr();
	...
	if($U['status']==8){
		...
		if(!isSet($_REQUEST['session'])) hidden('session', $U['session']);
...
function send_backup(){
	...
	if($_REQUEST['do']==='backup'){
		if(isSet($_REQUEST['settings'])){
			foreach($C['settings'] as $setting) $code['settings'][$setting]=get_setting($setting);
		}
...
function send_admin($arg=''){
	...
	foreach($P as $user){
		if($user[2]<$U['status']) $chlist.="<option value=\"$user[0]\" style=\"$user[1]\">$user[0]</option>";
	}
	$chlist.='</select>';
	...
	if(get_setting('suguests')){
		...
		foreach($P as $user){
			if($user[2]==1) echo "<option value=\"$user[0]\" style=\"$user[1]\">$user[0]</option>";
		}
...
function send_sessions(){
	...
	foreach($lines as $temp){
		if($temp['status']!=0 && $temp['entry']!=0 && (!$temp['incognito'] || $temp['status']<$U['status'])){
			...
			if($temp['status']>2) get_timeout($temp['lastpost'], $memexpire);
			else get_timeout($temp['lastpost'], $guestexpire);
			echo '</td>';
			if($U['status']>$temp['status'] || $U['session']===$temp['session']){
				echo "<td style=\"padding:5px;\">$temp[useragent]</td>";
				if($trackip) echo "<td style=\"padding:5px;\">$temp[ip]</td>";
				echo '<td style="padding:5px;">';
				frmadm('sessions');
				echo hidden('nick', $temp['nickname']).submit($I['kick']).'</form></td></tr>';
			}else{
				echo '<td style="padding:5px;">-</td>';
				if($trackip) echo '<td style="padding:5px;">-</td>';
...
function send_notes($type){
	...
	else echo "<h2>$I[adminnotes]</h2><p>";
...
function send_del_confirm(){
	...
	frmpst('delete');
	if(isSet($_REQUEST['multi'])) echo hidden('multi', 'on');
	if(isSet($_REQUEST['sendto'])) echo hidden('sendto', $_REQUEST['sendto']);
...
function send_post(){
	...
	echo '<option ';
	if($_REQUEST['sendto']==='*') echo 'selected ';
	...
	foreach($P as $user){
		if($U['nickname']!==$user[0] && !in_array($user[0], $ignored)){
			echo '<option ';
			if($_REQUEST['sendto']===$user[0]) echo 'selected ';
...
function send_profile($arg=''){
	...
	thr();
	if(!isSet($_COOKIE[$C['cookiename']])) $param="&session=$U[session]&lang=$C[lang]"; else $param='';
	...
	if($U['status']>=2){
		...
		echo "<tr><td><table style=\"border-spacing:0px;margin-left:auto;\"><tr><td>&nbsp;</td><td>$I[oldpass]</td><td><input type=\"password\" name=\"oldpass\" size=\"20\"></td></tr>";
		echo "<tr><td>&nbsp;</td><td>$I[newpass]</td><td><input type=\"password\" name=\"newpass\" size=\"20\"></td></tr>";
		echo "<tr><td>&nbsp;</td><td>$I[confirmpass]</td><td><input type=\"password\" name=\"confirmpass\" size=\"20\"></td></tr></table></td></tr></table></td></tr>";
		thr();
	}
...
function send_controls(){
	...
	if($U['status']>=5) echo "<td><$H[form] target=\"view\">$H[commonform]".hidden('action', 'admin').submit($I['adminbtn']).'</form></td>';
	if($U['status']>=5) echo "<td><$H[form] target=\"view\">$H[commonform]".hidden('action', 'notes').submit($I['notes']).'</form></td>';
	if($U['status']>=3) echo "<td><$H[form] target=\"_blank\">$H[commonform]".hidden('action', 'login').submit($I['clone']).'</form></td>';
...
function send_login(){
	...
	if($englobal!==1 || (isSet($_POST['globalpass']) && $_POST['globalpass']==get_setting('globalpass'))){
		...
		send_captcha();
		if($ga!==0){
			if($englobal===2) echo "<tr><td style=\"text-align:left;\">$I[globalloginpass]</td><td style=\"text-align:right;\"><input type=\"password\" name=\"globalpass\" size=\"15\"></td></tr>";
		...
		if($ga===0) echo "<tr><td colspan=\"2\">$I[noguests]</td></tr>";
		echo '<tr><td colspan="2">'.submit($I['enter']).'</td></tr></table></form>';
	}
	echo "<p>$I[changelang]";
	foreach($L as $lang=>$name){
...
function print_chatters(){
	...
	if(!empty($G)) echo "<th>$I[guests]:</th><td>&nbsp;</td><td>".implode(' &nbsp; ', $G).'</td>';
	echo '</tr></table>';
}
...
function create_session($setup){
	...
	if(!check_member()) add_user_defaults();
	$U['entry']=$U['lastpost']=time();
	if($setup) $U['incognito']=1;
...
function approve_session(){
	...
	if(isSet($_REQUEST['what'])){
		if($_REQUEST['what']==='allowchecked' && isSet($_REQUEST['csid'])){
			...
			foreach($_REQUEST['csid'] as $nick) $stmt->execute(array($time, $_REQUEST['kickmessage'], $nick));
		}elseif($_REQUEST['what']==='denyall' && isSet($_REQUEST['alls'])){
			$time=60*(get_setting('kickpenalty')-get_setting('guestexpire'))+time();
			$stmt=$db->prepare("UPDATE $C[prefix]sessions SET lastpost=?, status=0, kickmessage=? WHERE nickname=? AND status=1;");
			foreach($_REQUEST['alls'] as $nick) $stmt->execute(array($time, $_REQUEST['kickmessage'], $nick));
		}
	}
}
...
function parse_sessions(){
	global $C, $P, $U, $countmods, $db, $memcached;
...
function change_status(){
	...
	if($stmt->fetch(PDO::FETCH_ASSOC)){
		if($_REQUEST['set']==='-'){
			...
			$stmt->execute(array($_REQUEST['name']));
			if($C['memcached']) $memcached->delete("$C[dbname]-$C[prefix]members");
			if(isSet($P[$_REQUEST['name']])) $P[$_REQUEST['name']][2]=1;
			...
			$stmt->execute(array($_REQUEST['set'], $_REQUEST['name']));
			if($C['memcached']) $memcached->delete("$C[dbname]-$C[prefix]members");
			if(isSet($P[$_REQUEST['name']])) $P[$_REQUEST['name']][2]=$_REQUEST['set'];
...
function amend_profile(){
	...
	if(isSet($_REQUEST['timestamps'])) $U['timestamps']=1;
	else $U['timestamps']=0;
	if(isSet($_REQUEST['embed'])) $U['embed']=1;
	else $U['embed']=0;
	if($U['status']>=5 && isSet($_REQUEST['incognito']) && get_setting('incognito')) $U['incognito']=1;
	else $U['incognito']=0;
}
...
function apply_linkfilter(){
	...
	foreach($filters as $filter){
		$U['message']=preg_replace_callback("/<a href=\"([^\"]+)\" target=\"_blank\">([^<]+)<\/a>/i", function ($matched) use(&$filter){ return "<a href=\"$matched[1]\" target=\"_blank\">".preg_replace("/$filter[match]/i", $filter['replace'], $matched[2]).'</a>';}, $U['message']);
	}
	$redirect=get_setting('redirect');
	if(get_setting('imgembed')) $U['message']=preg_replace_callback('/\[img\]\s?<a href="([^"]+)" target="_blank">([^<]+)<\/a>/i', function ($matched){ return str_ireplace('[/img]', '', "<br><a href=\"$matched[1]\" target=\"_blank\"><img src=\"$matched[1]\"></a><br>");}, $U['message']);
	if(empty($redirect)) $redirect="$_SERVER[SCRIPT_NAME]?action=redirect&url=";
	if(get_setting('forceredirect')) $U['message']=preg_replace_callback('/<a href="([^"]+)" target="_blank">([^<]+)<\/a>/', function ($matched) use($redirect){ return "<a href=\"$redirect".urlencode($matched[1])."\" target=\"_blank\">$matched[2]</a>";}, $U['message']);
	elseif(preg_match_all('/<a href="([^"]+)" target="_blank">([^<]+)<\/a>/', $U['message'], $matches)){
		foreach($matches[1] as $match){
			if(!preg_match('~^http(s)?://~', $match)){
				$U['message']=preg_replace_callback('/<a href="('.str_replace('/', '\/', $match).')\" target=\"_blank\">([^<]+)<\/a>/', function ($matched) use($redirect){ return "<a href=\"$redirect".urlencode($matched[1])."\" target=\"_blank\">$matched[2]</a>";}, $U['message']);
			}
		}
	}
}
...
function print_messages($delstatus=''){
	...
	if(!empty($delstatus)){
		...
		while($message=$stmt->fetch(PDO::FETCH_ASSOC)){
			...
			if($injectRedirect){
				$message['text']=preg_replace_callback('/<a href="([^"]+)" target="_blank">([^<]+)<\/a>/', function ($matched) use ($redirect){ return "<a href=\"$redirect".urlencode($matched[1])."\" target=\"_blank\">$matched[2]</a>";}, $message['text']);
			}
			if($removeEmbed){
				$message['text']=preg_replace_callback('/<img src="([^"]+)"><\/a>/', function ($matched){ return "$matched[1]</a>";}, $message['text']);
			}
			echo "<div class=\"msg\"><input type=\"checkbox\" name=\"mid[]\" id=\"$message[id]\" value=\"$message[id]\"><label for=\"$message[id]\">";
			if($timestamps) echo ' <small>'.date($dateformat, $message['postdate']).' - </small>';
...
function init_chat(){
	...
	if(check_init()){
		...
		if($C['dbdriver']===0){//MySQL
			$db->exec("CREATE TABLE IF NOT EXISTS $C[prefix]captcha (id int(10) unsigned NOT NULL AUTO_INCREMENT, time int(10) unsigned NOT NULL, code char(5) NOT NULL, PRIMARY KEY (id) USING BTREE) ENGINE=MEMORY DEFUALT CHARSET=utf8 COLLATE=utf8_bin;");
...
function update_db(){
	...
	if($dbversion<$C['dbversion'] || get_setting('msgencrypted')!=$C['msgencrypted']){
		...
		if($dbversion<11){
			...
			while($temp=$result->fetch(PDO::FETCH_ASSOC)){
				if(isSet($F[$temp['fontface']])) $fontface=$F[$temp['fontface']];
				else $fontface='';
...
function check_db(){
	...
	try{
		if($C['dbdriver']===0) $db=new PDO("mysql:host=$C[dbhost];dbname=$C[dbname]", $C['dbuser'], $C['dbpass'], $options);
		elseif($C['dbdriver']===1) $db=new PDO("pgsql:host=$C[dbhost];dbname=$C[dbname]", $C['dbuser'], $C['dbpass'], $options);
		else $db=new PDO("sqlite:$C[sqlitedbfile]", NULL, NULL, $options);
...
function load_html(){
	...
	if(isSet($_REQUEST['session'])) $H['commonform'].=hidden('session', $_REQUEST['session']);
	$H=$H+array(
